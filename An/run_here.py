...
class RunHereCommand(sublime_plugin.WindowCommand):
    def run(self, path=None):
        ...
        if self.run_list_data:
            ...
            self.window.show_quick_panel(run_list, run_at(self.onselect, path), selected_index=an._run_here_last or -1)

    def onselect(self, index):
        if index == -1:
            return
        an._run_here_last = index
